...
/** Types a string through the real engine, which is what produces a timeline. */
...
describe('replay', () => {
  ...
  it('lets a later keystroke at the same index stand, the way a fix does', () => {
    const timeline: Keystroke[] = [
      ...
      // Backspace is not in the timeline; the correction reuses the index.
    ...
    // The mistake stays on the record even though the character was fixed.
